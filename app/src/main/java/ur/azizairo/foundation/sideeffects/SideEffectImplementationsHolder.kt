...
class SideEffectImplementationsHolder {
    ...
    fun <Mediator, Implementation> getWithPlugin(
    ...
    fun <Mediator, Implementation> putWithPlugin(
        ...
        val sideEffectMediator = sideEffectMediatorsHolder.get(plugin.mediatorClass)
        val target = plugin.createImplementation(sideEffectMediator)
        if (target != null && target is SideEffectImplementation) {
            _implementations[plugin.mediatorClass] = target
            target.injectActivity(activity = activity)
        }
    }
